...
# A subagent's transcript lives at ``<session>/subagents/*.jsonl`` and carries the
# ``isSidechain`` / ``agentId`` envelope no top-level session has, so the one session
# yields two files. That split is claude's, not a choice here -- every other record kind
# lands in the single main file.
#
# ``--permission-mode bypassPermissions``, not ``--dangerously-skip-permissions``: both
# skip per-tool prompts, but the latter shows a one-time acceptance dialog on first use
# in a TUI, and the first scripted line answers THAT instead of being submitted as a
# turn. Either way an unanswered prompt blocks on a keypress the script never sends. The
...
# ``model_reasoning_summary=detailed`` is what populates a ``reasoning`` item's
...
# The TUI is the only interface that reaches every record kind: a slash command is
# handled inside the CLI and never becomes an argument, so ``-p`` / ``codex exec``
# cannot produce a model change or a compaction at all.
# :class:`~trackinizer.lib.posix.relay.ThreadedRelay` owns the master fd, which makes an
# injected line indistinguishable from a typed one -- bracketed paste, then Enter as its
# own delayed write.
#
# Synchronization is on the session log, not the clock. The relay mirrors bytes without
# parsing them, so the TUI offers no readback to wait on -- but the CLI appends to its
# session file as a turn progresses, and that file is exactly what this script captures.
# Waiting for the log to go quiet is therefore a direct measure of "the turn finished".
#
# A fixed sleep was tried first and silently lost the first two turns: they were
# submitted before the TUI had drawn its prompt, and a line typed into a terminal that
# is not ready is discarded with no error anywhere. Waiting for the log to appear
# removes that whole class of failure -- the file cannot exist until the session does.
def _drive(
    argv: list[str],
    *,
    cwd: Path,
...
# :class:`~trackinizer.lib.posix.relay.ThreadedRelay`, which caused both failures this capture
# has hit: ``ANTHROPIC_API_KEY`` made claude open a "use this key?" prompt at boot that
# ate the first two turns, and a codex run read a live key out of the environment and
# wrote it into its own transcript.
#
# An allowlist rather than a denylist, because the leak is the variable nobody thought
# to name -- ``OPENAI_API_KEY`` and every future ``*_TOKEN`` a tool learns to read are
# excluded without being listed. Authentication comes from the credentials copied into
# the throwaway config root, so nothing here needs a secret.
#
# ``PWD`` is deliberately absent: the caller chdirs first, and claude names its session
# directory after the resolved working directory, so a stale ``PWD`` would file the
# capture's log under the operator's shell cwd.
...
# Claude closes each turn with a ``system`` / ``turn_duration`` record, so counting
# those is an exact end-of-turn signal rather than an inference. Polling for silence
# instead cost the whole quiet window on EVERY turn -- seven turns of a 20s window added
# ~140s to a session the model finishes in about 50 -- and still ended turns early,
# because a window short enough to be cheap is shorter than the 13.5s gaps that occur
# inside one turn.
#
# ``quiet_sec`` is the fallback for a CLI that writes no such marker (codex, and any
# turn claude ends without one): the log going quiet for that long is then the only
# available signal.
...
# Read as raw text rather than parsed: this runs twice a second while the CLI appends,
# so a partially-written final line is expected, and counting a marker substring cannot
# fail on one.
...
# Credentials are copied rather than shared: the CLI rewrites files in its config root
# (session indexes, caches), and pointing it at the operator's real one is what a
# hermetic capture exists to avoid.
...
# ``-p`` skips these; the TUI does not, and each one absorbs a scripted line as a menu
# keystroke, so the capture silently runs turns short. Two dialogs exist and both must
# be seeded:
#
# * onboarding -- an unseeded root opens the OAuth login wizard instead of a prompt, so
# no session is ever written; * the per-workspace trust dialog ("Is this a project you
# trust?"), which fires on any directory claude has not seen -- and a throwaway
# workspace is new every run by construction.
#
# Every key but ``projects``, not a hand-picked few. A three-key subset was what this
# seeded first, and driven against the real binary it HANGS: the CLI wants ~80 further
# acknowledgement and migration flags, and paints a dialog for a missing one rather than
# reading the message. ``projects`` is the one key genuinely worth dropping -- it is 133
# KB of the operator's own per-directory history, and a run without it answers
...
# The throwaway directories are named after a PID-unique temp path, so an unredacted
# fixture would differ on every capture and leak the operator's layout. The home
# directory is redacted too, and not only for the CLI's own envelope fields: a tool the
# model ran can print an absolute path from anywhere on the machine into its output (a
# Python traceback naming a site-packages file is how this was found).
#
# Account and organization identifiers are replaced too: a PTY capture writes a
# ``bridge-session`` record carrying ``ownerAccountUuid`` and ``ownerOrganizationUuid``,
# which name the operator's real account. Each is swapped for a fixed UUID, so the
# fixture keeps the field's shape -- the thing a parser is tested against -- without its
# value.
#
# Every replacement is a JSON-safe literal of the same syntactic shape, so substituting
# in the raw text keeps each line parseable without re-serializing -- which would
# rewrite the CLI's own byte formatting, the thing under test.
...
# The filler repeats ``deadbeef`` to the length of what it replaces and keeps the
# leading marker (``sk-``, ``ghp_``), so the record still looks like the thing the
# parser must handle: a fixed short token would change both the field's length and its
# shape, and a 1900-character key is exactly the case worth keeping in a fixture.
...
# Filler is exempt, and cannot be confused for a real secret: it is ``deadbeef``
# repeated, which no issuer produces. Without the exemption this check fires on the
# scrub's own output, since the replacement keeps the shape that matched.
