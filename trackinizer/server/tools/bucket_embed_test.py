"""Length-bucketed embed core: routing, tail-pad, shape invariant, order.

Every test uses a FAKE tokenizer (an injectable text->token-count rule) and a
FAKE ONNX session (records the ``(rows, seq)`` shape of every forward) -- no
...
class _FakeTokenizer:
    ...
    ``count`` is how ``embed_bucketed`` learns each text's TRUE length (the
    routing key). A test can make "dense" text have more tokens than
    ...
    def pad_batch(
        ...
        *,
        ...
        of the same shape -- the fake session reads only shapes, not token values.
        """
        mask = np.zeros((rows, seq), dtype=np.int64)
        for i, text in enumerate(texts):
            count = min(self._token_count(text), seq)
            mask[i, seq - count :] = 1
        return {
            "input_ids": np.zeros((rows, seq), dtype=np.int64),
            "attention_mask": mask,
        }
...
class _FakeSession:
    ...
    def run(
        ...
        """Return a fake ``last_hidden_state`` of ``(rows, seq, dim)``."""
        del output_names
        ...
        hidden = np.zeros((rows, seq, self.dim), dtype=np.float32)
        for i in range(rows):
...
async def test_dense_text_routes_by_token_count_not_chars() -> None:
    ...
    seqs = {seq for _rows, seq in session.shapes}
    assert seqs == {512}
...
async def test_every_forward_shape_is_in_the_declared_set() -> None:
    ...
    The core invariant: a bounded set of exactly ``len(edges)`` shapes. A ragged
    batch must row-pad to its bucket's full ``rows[edge]``. Breaking the tail-pad
    (see the mutation proof) makes a forward appear at a non-declared row count
    and fails this.
    """
    ...
    assert set(session.shapes) <= declared, (session.shapes, declared)
...
async def test_tail_rows_are_padded_and_sliced_back() -> None:
    ...
    rows_seen = {rows for rows, _seq in session.shapes}
...
async def test_vectors_return_in_input_order_across_buckets() -> None:
    ...
    The fake session stamps each row's ordinal into its vector; after routing
