...
the same shape: a lazy ONNX Runtime session load behind a module ``_load`` seam
(so fake-model tests never download weights), ``embed`` / ``embed_batch`` /
...
zero-arg factory so config-time imports pull no onnxruntime.
"""
